...
const AddReservation = () => {
    ...
    const handleStartDateChange = (date) => {
        const parsedStartDate = parse(date, 'yyyy-MM-dd', new Date());
        const parsedEndDate = parse(formData.endDate, 'yyyy-MM-dd', new Date());
    
        if (formData.endDate && isBefore(parsedStartDate, parsedEndDate)) {
            setFormData({ ...formData, startDate: formData.endDate, endDate: date });
            calculateTotal(formData.endDate, date);
        } else {
            setFormData({ ...formData, startDate: date });
            calculateTotal(date, formData.endDate);
        }
    };
    
    const handleEndDateChange = (date) => {
        const parsedStartDate = parse(formData.startDate, 'yyyy-MM-dd', new Date());
        const parsedEndDate = parse(date, 'yyyy-MM-dd', new Date());
    
        if (isBefore(parsedStartDate, parsedEndDate) || parsedStartDate.toISOString() === parsedEndDate.toISOString()) {
