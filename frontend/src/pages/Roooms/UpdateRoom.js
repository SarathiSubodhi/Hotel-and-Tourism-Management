...
const UpdateRooms = () => {
    ...
    useEffect(() => {
        const fetchRoomData = async () => {
            try {
                const response = await axios.get(`http://localhost:8070/rooms/get/${id}`);
    ...
    const calculateTotal = () => {
        if (formData.roomType && formData.startDate && formData.endDate) {
            ...
            // Update only the total field in formData without resetting the entire state object
            setFormData(prevState => ({ ...prevState, total: totalAmount }));
        }
    };
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`http://localhost:8070/rooms/update/${id}`, formData);
            navigate('/viewrooms');
        } catch (error) {
